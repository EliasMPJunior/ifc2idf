...
pset_logger = logging.getLogger('bim2sim.ifc_property_set')
pset_logger.propagate = False
_pset_log_file = Path.cwd() / 'ifc_property_set.log'
if not any(isinstance(h, logging.FileHandler) and getattr(h, 'baseFilename', None) == str(_pset_log_file) for h in pset_logger.handlers):
    _handler = logging.FileHandler(_pset_log_file, encoding='utf-8')
    _handler.setFormatter(logging.Formatter('%(asctime)s %(message)s'))
    pset_logger.addHandler(_handler)
    pset_logger.setLevel(logging.INFO)
...
def property_set2dict(property_set: entity_instance,
    ...
    pset_logger.info(f"property_set: {property_set}")
